...
const TaskPage = () => {
  ...
  useEffect(() => {
    ...
    setCurEmployeeId(parseInt(employeeId, 10));
  ...
  const fetchManagerDetails = async (managerId) => {
    try {
      const response = await fetch(
        `http://localhost:8080/employees/${managerId}`
      );
      const managerData = await response.json();
      setTask((prevTask) => ({ ...prevTask, assignedBy: managerData.employeeId }));
    } catch (error) {
  ...
  const fetchEmployees = async (managerId) => {
    try {
      const response = await fetch(
        `http://localhost:8080/employees/org/${managerId}`
      );
      const data = await response.json();
      setEmployees(data);
    } catch (error) {
  ...
  const removeEmployee = (employeeId) => {
    fetch(`http://localhost:8080/employees/remove/${employeeId}`, {
      method: "DELETE",
    })
      .then((response) => {
        if (response.ok) {
      ...
      <ul className="task-list">
        {tasks.map((task) => (
          <li key={task.taskId} className="task-item">
            <h3 style={{ width: "25%" }} className="good">
              {task.taskName}
            </h3>
            <p className="task-description" style={{ width: "25%" }}>
              {task.description}
            </p>
  ...
  const renderManagerTaskView = () => (
    <div className="employee-view">
      <button className="nav-button" onClick={() => setTaskPage(false)}>
        {"<"} Go back
      </button>
      <h2 className="main-page-title">Your Tasks</h2>
      <ul className="task-list">
  ...
  return (
    <div className="task-container">
      {showNotification && <Notification message={`Employee Deleted!`} />}
      {role === "Manager"
        ? taskpage
          ? renderManagerTaskView()
          : renderManagerView()
        : !taskpage
        ? renderEmployeeTaskView()
        : renderEmployeeView()}
